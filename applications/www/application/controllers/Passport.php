...
//                         'Model_tel_verify' => 'Muser_tel_verify',
...
//                         'Model_access_token' => 'Maccess_token'
    ...
    public function get_access_token(){
        ...
        //获取基础支持的access_token 基础支持的access_token每天只能获取2000次，在文件中存储，失效后重新获取
    ...
    public function wechat_login(){
        ...
        $state = $this->input->get('state');
        //防止手机端登陆后，PC端扫码无效
        if($state=='mobile' && isset($data['userInfo']) && !empty($data['userInfo']['tel'])){
            redirect($data['domain']['mobile']['url'].'/home');
            exit;
        }
        $wechat_user_info = $this->get_wechart_user_info();
        if(isset($wechat_user_info['errcode'])){
        ...
        if(!empty($wechat_user_info))
        {
            ...
            if($user_info && !empty($user_info['tel']))
            {
        ...
        {
            //没有获取到微信信息 提示重新登陆
            $this->return_failed('没有获取到微信信息 提示重新登陆');
        }
    }
    ...
    public function bind_account(){
        ...
        //验证短信验证码
        $code = $this->Muser_tel_verify->get_one('code', array('mobile' => $post_data['mobile']));
        ...
        $exist_data = $this->Muser->get_one($field, array('tel' => $post_data['mobile'], 'is_del' => 0));
        ...
        if($exist_data){
            ...
            if($open_id_exist){
                //手机号已绑定，微信号已绑定：删除已绑定微信的账号
                $this->Muser->update_info(array('is_del' => 1), array('open_id' => $post_data['open_id']));
            }
            //手机号已绑定，微信号未绑定： 绑定微信
            $this->Muser->update_info($insert_data, array('tel' => $post_data['mobile']));
            $bind_success = true;
        }else{
            //如果存在用户的open_id记录，则删除条这条信息
            if($open_id_exist){
                //手机号没绑定，微信号已绑定： 绑定手机
                $this->Muser->update_info(array('tel' => $post_data['mobile']), array('open_id' => $post_data['open_id']));
            }else{
                //没绑定手机，没绑定微信： 创建新账号
        ...
        if($bind_success){
            ...
            if(!empty($post_data['state']) && $post_data['state'] != 'mobile'){
                ...
                //手机端
                $user_info = $this->Muser->get_one('id,tel,person_type', array('open_id' => $post_data['open_id'], 'is_del' => 0));
                if($user_info){
