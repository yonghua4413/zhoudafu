...
(function() {
	...
	var dummyStyle = document.createElement('div').style,
		...
		handleEvent: function (e) {
			if (e.type == START_EV) {
				...
				if (e.currentTarget.id == 'sw-cancel' || e.currentTarget.id == 'sw-done') {
					...
					this.scrollStart(e);
				}else if(e.currentTarget.id == 'maskDate') {
					this.destroy();
					$('#maskDate').hide();
				}
		...
		onOrientationChange: function (e) {
			window.scrollTo(0, 0);
			this.swWrapper.style.top = (this.container ? this.container.clientHeight : (window.innerHeight + window.pageYOffset)) + disTop+ 'px';
		...
		create: function () {
			...
			div.style.top = (this.container ? this.container.clientHeight : (window.innerHeight + window.pageYOffset)) + disTop +'px';		// Place the SW down the actual viewing screen
			div.style[transitionProperty] = hasTransform ? cssVendor + 'transform' : 'top left';
			div.style[transitionDuration] = '400ms';
			if(!hasTransform) {
				div.style.position = 'absolute';
			}
			div.innerHTML = '<div id="sw-header"><div id="sw-cancel">取消</' + 'div><span id="sw-title-info">请提前30分钟预约</'+'span><div id="sw-done">确定</' + 'div></' + 'div><div id="sw-slots-wrapper"><div id="sw-slots"></' + 'div></' + 'div><div id="sw-frame"></' + 'div>';
			// div.innerHTML = '<div id="sw-header"><button id="sw-cancel">取消</' + 'button><span id="sw-title-info">请提前30分钟预约</'+'span><button id="sw-done">确定</' + 'button></' + 'div><div id="sw-slots-wrapper"><div id="sw-slots"></' + 'div></' + 'div><div id="sw-frame"></' + 'div>';
			// if(num==1) {
			// 	$('#sw-number').show();
			// 	$(div).append('<div id="sw-number"><div id="sw-number-top"><span>出行人数</span><em><i class="decrement"></i><strong>1</strong><i class="increment"></i></em></div><p>为了在购买保险时保障您的合法权益，人数请包括婴幼儿</p></' + 'div>');
   //          }
			...
			document.getElementById('maskDate').addEventListener(START_EV, this, false);
   ...
   //          }
		...
		destroy: function () {
			...
			document.getElementById('maskDate').removeEventListener(START_EV, this, false);
