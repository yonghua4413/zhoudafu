...
class MY_Controller extends CI_Controller {
     ...
     * louhang@gz-zc.cn
     */
    public function setActiceMenu(){
        $method =  $this->uri->segment(2) ? strtolower($this->uri->segment(2)) : 'index'; //方法
        ...
        foreach ($this->data['menu'] as $k => $v){
            foreach ($v['list'] as $k2 => $v2){
                if($operation == $v2[0] || $operation == $v2[0].'/index'){
                    $_SESSION['left_active_code'] = $v['code'];
                    $_SESSION['left_active_method'] = $v2[0];
                    break;
                }
            }
        }
        $this->data['left_active_code'] = isset($_SESSION['left_active_code']) ? $_SESSION['left_active_code'] : '';
        $this->data['left_active_method'] = isset($_SESSION['left_active_method']) ? $_SESSION['left_active_method'] : '';
    }
