...
class Userextend extends MY_Controller{
    ...
    public function info(){
        try {
            ...
            if(!empty($mobile) && !empty($user_id)){
                $result = $this->Muser_extend->get_one($field,['mobile'=>$mobile, 'or' => array('user_id' => $user_id)]);
                ...
                $result = $this->Muser_extend->get_one($field, ['mobile' => $mobile]);
            }
            
            $result['card_front'] = explode(',', $result['card_front']);
            $result['card_back'] = explode(',', $result['card_back']);
            $result['edu_certificate'] = explode(',', $result['edu_certificate']);
            if(!empty($result)){
