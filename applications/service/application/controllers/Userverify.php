...
class Userverify extends MY_Controller {
	...
	public function get_tel_code_info(){
	    ...
	    $data = array();
	    if ($mobile){
	        $data = $this->Muser_tel_verify->get_one(array('id', 'tel', 'code', 'add_time'), array('tel'=>$mobile));
	    } 
	...
	public function update_tel_code_info($code = 0, $id = 0){
	    try {
    	    ...
    	    $result = $this->Muser_tel_verify->update_info(array( 'code'=>$code,'add_time'=>time()), array('id'=>$id));
	        if($result){
	            $this->return_success();
	        }else{
	            $this->return_failed();
	...
	public function add(){
	    try {
	        ...
	        $result = $this->Muser_tel_verify->create($data);
	        if($result){
